...
W modelu stworzonym za pomoca funkcji `lda()`, użyto wszytkich zmiennych z tabeli (64), chociaż niektóre z nich mają bardzo niskie współczynniki. Postanowiłam wykorzystać nwszytskie zmienne ponieważ wyniki z tylko 5 z modelu Altmana były jeszcze bardziej rozczarowywujące.
...
model_lda <- lda(X0 ~ ., data = nauka)

predictions <- predict(model_lda, newdata = testowe[,1:64])
...
matrixc <- data.frame(c(37,15), c(13,35))
...
Dokładnośc modelu Tutsona plasuje sie na wysokości **72%**, co czyni go nieznacznie lepszym od modelu Altmana.Taki wynik nadal pozostaje niewystarczający, a liniowa analiza dyskryminacyjna okazuje się nienajlepszym podejściem do zagadnienia ryzyka kredytowego.
